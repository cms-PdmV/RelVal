...
class RelValController(ControllerBase):
    ...
    def get_cmsdriver(self, relval, for_submission=False):
        ...
        return relval.get_cmsdrivers(for_submission)
    ...
    def get_config_upload_file(self, relval):
        ...
        database_url = CMSWEB_URL.replace("https://", "").replace("http://", "")
        bash = ["#!/bin/bash", ""]
        ...
        for step in steps:
            ...
            if commands and (
                step_cmssw != previous_cmssw or scram_arch != previous_scram
            ):
                commands += [""]
                bash += run_commands_in_cmsenv(
                    commands, previous_cmssw, previous_scram
                ).split("\n")
                commands = []

            commands.append(
                (
                    "$PYTHON_INT config_uploader.py "
    ...
    def get_task_dict(self, relval, step, step_index):
        ...
        if step_index == 0:
            ...
            if events_per_lumi:
                ...
                task_dict["EventsPerLumi"] = min(
                    int(events_per_lumi), int(events_per_job)
                )
            else:
    ...
    def get_job_dict(self, relval):
        ...
        job_dict["CouchURL"] = CMSWEB_URL + "/couchdb"
        ...
        if job_dict_overwrite:
            self.logger.info(
                "Overwriting job dict for %s with %s", prepid, job_dict_overwrite
            )
    ...
    def resolve_auto_conditions(self, conditions_tree):
        ...
        for cmssw_version, scram_tree in conditions_tree.items():
            for scram_arch, conditions in scram_tree.items():
                ...
                resolve_command += run_commands_in_cmsenv(
                    [
                        "$PYTHON_INT resolve_auto_global_tag.py "
                        f'"{cmssw_version}" '
                        f'"{scram_arch}" '
        ...
        self.logger.debug(
            "Resolve auto conditions command:\n%s", "\n".join(resolve_command)
        )
        with SSHExecutor(
            ...
            ssh_executor.upload_as_file(
                "\n".join(resolve_command), f"{remote_directory}/{script_name}"
            )
            command = [
    ...
    def next_status(self, relvals):
        ...
        for status, relvals_with_status in by_status.items():
            self.logger.info(
                "%s RelVals with status %s", len(relvals_with_status), status
            )
            if status == "new":
    ...
    def move_relvals_to_done(self, relvals):
        ...
        for relval in relvals:
            ...
            with self.locker.get_nonblocking_lock(prepid):
                ...
                not_valid_datasets = [
                    d["name"] for d in datasets if d["type"].lower() != "valid"
                ]
    ...
    def update_workflows(self, relval):
        ...
        with self.locker.get_lock(prepid):
            ...
            self.logger.info(
                "%s workflows that are not in stats: %s",
                len(workflow_names),
                workflow_names,
            )
    ...
    def get_output_datasets(self, relval, all_workflows):
        ...
        self.logger.info(
            "%s output datatiers are: %s", prepid, ", ".join(output_datatiers)
        )
        ...
        self.logger.debug(
            "Output datasets tree:\n%s",
            json.dumps(output_datasets_tree, indent=2, sort_keys=True),
        )
        output_datasets = []
        ...
        self.logger.debug(
            "Output datasets:\n%s",
            json.dumps(output_datasets, indent=2, sort_keys=True),
        )
